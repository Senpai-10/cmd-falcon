...
*/
class Cli {
    constructor(cli_name, description, version, config, args) {
        this.opts = {};
    ...
    parse() {
        ...
        for (let i = 0; i < this.args.length; i++) {
            ...
            if (arg.startsWith('--')) {
                ...
                let option = (0, utils_1.find_option)(name, 'long', this.options);
                if (option == undefined)
                    continue;
                if (option.is_flag) {
                    this.opts[name] = true;
                ...
                if (arg.length > 1) {
                    for (const short_option of arg) {
                        let option = (0, utils_1.find_option)(short_option, 'short', this.options);
                        if (option == undefined)
                            continue;
                    ...
                    let option = (0, utils_1.find_option)(arg, 'short', this.options);
                    if (option == undefined)
                        continue;
                    let name = option.name.long.substring(2);
                    if (option.is_flag) {
